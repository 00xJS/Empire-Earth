...
SAVED_VD_SIZE="${VIRTUAL_DESKTOP_SIZE:-1024x801}"
...
updates = {
    ...
    "DirectXExt": {
        ...
        "ExtraEnumeratedResolutions": "800x600,1024x801",
...
install_d7vk() {
  ...
  #
  # shaderCompilationMethod=async is the load-bearing line: with the default
  # (sync) the game's main thread parks after uploading its textures while the
  # DXVK compiler threads churn, the state cache stays at 12 bytes, and no frame
  # is ever presented -- the "fullscreen then blank" symptom.
...
dxvk.shaderCompilationMethod = async
...
reg "HKCU\\Software\\Wine\\Direct3D" "csmt" "REG_SZ" "0"
...
if [[ "${VIRTUAL_DESKTOP:-0}" == "1" ]]; then
  ...
  reg "HKCU\\Software\\Wine\\Explorer\\Desktops" "EmpireEarth" "REG_SZ" "${VIRTUAL_DESKTOP_SIZE:-1024x801}"
  ...
  reg "HKCU\\Software\\Wine\\Explorer\\Desktops" "Default" "REG_SZ" "${VIRTUAL_DESKTOP_SIZE:-1024x801}"
  reg "HKCU\\Software\\Wine\\Explorer" "Desktop" "REG_SZ" "EmpireEarth"
  log "Virtual desktop EmpireEarth ${VIRTUAL_DESKTOP_SIZE:-1024x801} enabled for Empire Earth.exe"
else
...
reg "HKCU\\Software\\SSSI\\Empire Earth" "Game Window Width" "REG_DWORD" "1024"
reg "HKCU\\Software\\SSSI\\Empire Earth" "Game Window Height" "REG_DWORD" "768"
